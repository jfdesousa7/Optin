<?php session_start(); include "../conex.php" ;

$consulta=mysqli_fetch_array(mysqli_query($link,"select * from proyectos where id=".$_GET["id"].""));

?>
<!DOCTYPE html>
<!--[if IE 8]> <html lang="en" class="ie8"> <![endif]-->
<!--[if !IE]><!-->
<html lang="en">
<!--<![endif]-->
<head>
	<meta charset="utf-8" />
	<title>Proyectos  editar</title>
	<meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" name="viewport" />
	<meta content="" name="description" />
	<meta content="" name="author" />
	
	<!-- ================== BEGIN BASE CSS STYLE ================== -->
	<link href="assets/plugins/jquery-ui/themes/base/minified/jquery-ui.min.css" rel="stylesheet" />
	<link href="assets/plugins/bootstrap/css/bootstrap.min.css" rel="stylesheet" />
	<link href="assets/plugins/icon/themify-icons/themify-icons.css" rel="stylesheet" />
	<link href="assets/css/animate.min.css" rel="stylesheet" />
	<link href="assets/css/style.min.css" rel="stylesheet" />
	<link href="assets/plugins/form/bootstrap-datepicker/css/bootstrap-datepicker.min.css" rel="stylesheet" />
	<link href="assets/plugins/form/bootstrap-timepicker/css/bootstrap-timepicker.min.css" rel="stylesheet" />
	<link href="assets/plugins/form/bootstrap-colorpicker/css/bootstrap-colorpicker.min.css" rel="stylesheet" />
	<link href="assets/plugins/form/bootstrap-tagsinput/css/bootstrap-tagsinput.css" rel="stylesheet" />
	<link href="assets/plugins/form/bootstrap-select/css/bootstrap-select.min.css" rel="stylesheet" />
	<link href="assets/plugins/form/bootstrap-slider/css/bootstrap-slider.css" rel="stylesheet" />
	
	<!-- ================== BEGIN PAGE CSS STYLE ================== -->
	<link href="assets/plugins/form/bootstrap-tagsinput/css/bootstrap-tagsinput.css" rel="stylesheet" />
	<link href="assets/plugins/form/summernote/summernote.css" rel="stylesheet" />
	<!-- ================== END PAGE CSS STYLE ================== -->
	<!-- ================== END BASE CSS STYLE ================== -->
	
	<!-- ================== BEGIN BASE JS ================== -->
	<script src="assets/plugins/loader/pace/pace.min.js"></script>
	<!-- ================== END BASE JS ================== -->
</head>
<body>
	<!-- BEGIN #page-container -->
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		<!-- BEGIN #header -->
		<?php  include "header.php";  ?>
		<!-- END #header -->
		
		<!-- BEGIN #sidebar -->
		<?php  include "sidebar.php"; ?>
		<!-- END #sidebar -->
		
		<!-- BEGIN #content -->
		<div id="content" class="content">
			<!-- BEGIN breadcrumb -->
			<ul class="breadcrumb">
				<li><a href="home.php">Home</a></li>
				<li><a href="proyectos.php">Proyectos </a></li>
				<li class="active">Proyectos  [Editar]</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			 
			<!-- END page-header -->
			<!-- BEGIN wizard -->
			<div id="rootwizard" class="wizard wizard-full-width">
				<!-- BEGIN wizard-header -->
				<div class="wizard-header">
					<ul class="nav nav-pills">
						<li class="active"><a href="#tab1" data-toggle="tab">Proyectos  - EDITAR</a></li>
						
								
					</ul>
				</div>
				<!-- END wizard-header -->
				<!-- BEGIN wizard-form -->
<form action="proyectos.php" method="post" enctype="multipart/form-data" onSubmit="$('#btn_editar').html('Editando ...')" name="wizard_form">
                              
           <input type="hidden" name="id" value="<?=$consulta["id"]?>">                      
                               
					<!-- BEGIN wizard-content -->
					<div class="wizard-content tab-content">
						<!-- BEGIN tab-pane -->
						<div class="tab-pane active" id="tab1">
							<!-- BEGIN row -->
							<div class="row">
								<!-- BEGIN col-7 -->
								<div class="col-md-7 col-md-offset-3">
									<p class="desc m-b-20">Proyectos  [Editar].</p>
									
							
<div class="form-group">
	<?php  if ($consulta["estatus"]=='si'){?>   <div class="switcher switcher-success">
	<input type="checkbox" name="estatus" id="estatus" checked="" value="si" />
	<label for="estatus"></label>
	</div>  <?php }else{ ?>
	 <div class="switcher switcher-success">
	<input type="checkbox" name="estatus" id="estatus"  value="no" />
	<label for="estatus"></label>
	</div> <?php } ?>	
</div>	
				
									
<ul class="nav nav-tabs" id="nav-tabs">
								<li class="active"><a href="#es" data-toggle="tab" aria-expanded="true">Datos</a></li>
								
</ul>

								<div class="tab-content tab-content-bordered">
								<!-- BEGIN tab-pane -->
								<div class="tab-content tab-content-bordered">
								<!-- BEGIN tab-pane -->
								<div class="tab-pane fade active in" id="es">
								
<div class="form-group">
<label class="control-label">Tipo</label>
			<select required name="id_tipo" >

			<?php  $sql=mysqli_query($link,"select * from proyectos_tipo where estatus='si'");
					  while($row=mysqli_fetch_array($sql)){?>
					  <option value="<?=$row["id"]?>" <?php   if($consulta["id_tipo"]==$row["id"]){echo "selected";} ?>><?=$row["nombre"]?></option>
					  <?php }
			 ?>

			</select>
</div>	

	
								
								
<div class="form-group">
										<label class="control-label"><span style="color:red">Nombre Proyecto</span></label>
<input required type="text" name="nombre" id="nombre" class="form-control" value="<?=$consulta["nombre"]?>">
</div>	


<div class="form-group">
										<label class="control-label">Descripción</label>
<textarea type="text" name="descripcion" id="descripcion" class="form-control summernote" rows="6" ><?=$consulta["descripcion"]?></textarea>
									</div>
	
	
	
<div class="form-group">
										<label class="control-label">Ubicación</label>
<input  type="text" name="ubicacion" id="ubicacion" class="form-control" value="<?=$consulta["ubicacion"]?>">
</div>	
<br><br>	
	
<div style="padding:2%;background-color:#f5f5f5">	
		<div class="form-group">
			<label class="control-label">Video URL (youtube Embed)</label>
			<input  type="text" name="video" id="" class="form-control" value="<?=$consulta["video"]?>">
		</div>	
	
		
</div>
<br><br>
<div class="form-group">
										<label class="control-label">Link 360 Tour</label>
<input  type="text" name="link_360" id="link_360" class="form-control" value="<?=$consulta["link_360"]?>">
</div>	



<div class="form-group">
										<label class="control-label">Link Brochure</label>
<input  type="text" name="link_brochure" id="link_brochure" class="form-control" value="<?=$consulta["link_brochure"]?>">
</div>	






<div class="form-group">
										<label class="control-label">Metraje</label>
<textarea type="text" name="metraje" id="metraje" class="form-control summernote" rows="6" ><?=$consulta["metraje"]?></textarea>
</div>	


<div class="form-group">
										<label class="control-label">Detalles</label>
<textarea type="text" name="detalles" id="detalles" class="form-control summernote" rows="6" ><?=$consulta["detalles"]?></textarea>
</div>	


<div class="form-group">
										<label class="control-label">Amenidades</label>
<textarea type="text" name="amenidades" id="amenidades" class="form-control summernote" rows="6" ><?=$consulta["amenidades"]?></textarea>
</div>	




<div class="form-group">
										<label class="control-label">Imágen Referencial</label>
										
										
													<input type="file" name="foto" > 
                                   </div>	
								   
								   
								   
								   
<div class="form-group">
<label class="control-label">Posición Portada</label>
			<select required name="ubicacion_portada" >

			 
				 <option value="center center" <?php   if($consulta["ubicacion_portada"]=='center center'){echo "selected";} ?>>Center</option>
				 <option value="top center" <?php   if($consulta["ubicacion_portada"]=='top center'){echo "selected";} ?>>Top</option>
				 <option value="bottom center" <?php   if($consulta["ubicacion_portada"]=='bottom center'){echo "selected";} ?>>Bottom</option>
				 <option value="left" <?php   if($consulta["ubicacion_portada"]=='left'){echo "selected";} ?>>Left</option>
				 <option value="right" <?php   if($consulta["ubicacion_portada"]=='right'){echo "selected";} ?>>Right</option>
					   

			</select>
</div>									   
								   


<div class="modal modal-cover fade" id="modal1">
								<div class="modal-dialog">
									<div class="modal-content">
										 
										<div class="modal-body">
										<center><h3>Espere por favor ...</h3></center>
										</div>
										<div class="modal-footer">
										</div>
									</div>
								</div>
							</div>

	

								</div>
								
								
								
							</div>	
									
								</div>
								<!-- END col-7 -->
							</div>
							<!-- END row -->
						</div>
						
						<!-- BEGIN wizard-footer -->
						<div class="wizard-footer">
							
<button style="float:right" type="submit" class="btn btn-primary  btn-rounded" name="btn_editar" id="btn_editar" >Guardar <i class="ti-pencil"></i> </button>
						</div>
						<!-- END wizard-footer -->
					</div>
					<!-- END wizard-content -->
					
				</form>
				<!-- END wizard-form -->
			</div>
			<!-- END wizard -->
		</div>
		<!-- END #content -->
		
		<!-- BEGIN btn-scroll-top -->
		<a href="#" data-click="scroll-top" class="btn-scroll-top fade"><i class="ti-arrow-up"></i></a>
		<!-- END btn-scroll-top -->
	</div>
	<!-- END #page-container -->
	
	
	<!-- ================== BEGIN BASE JS ================== -->
	<script src="assets/plugins/jquery/jquery-1.9.1.min.js"></script>
	<script src="assets/plugins/jquery/jquery-migrate-1.1.0.min.js"></script>
	<script src="assets/plugins/jquery-ui/ui/minified/jquery-ui.min.js"></script>
	<script src="assets/plugins/cookie/js/js.cookie.js"></script>
	<script src="assets/plugins/bootstrap/js/bootstrap.min.js"></script>
	<script src="assets/plugins/scrollbar/slimscroll/jquery.slimscroll.min.js"></script>
	<!-- ================== END BASE JS ================== -->
	<script src="assets/plugins/form/bootstrap-datepicker/js/bootstrap-datepicker.min.js"></script>
	<script src="assets/plugins/form/bootstrap-timepicker/js/bootstrap-timepicker.min.js"></script>
	<script src="assets/plugins/form/bootstrap-colorpicker/js/bootstrap-colorpicker.min.js"></script>
	<script src="assets/plugins/form/bootstrap-typeahead/js/bootstrap-typeahead.min.js"></script>
	<script src="assets/plugins/form/bootstrap-tagsinput/js/bootstrap-tagsinput.min.js"></script>
	<script src="assets/plugins/form/bootstrap-slider/js/bootstrap-slider.min.js"></script>
	<script src="assets/plugins/form/bootstrap-select/js/bootstrap-select.min.js"></script>
	<script src="assets/plugins/form/masked-input/js/masked-input.min.js"></script>
	<script src="assets/plugins/form/pwstrength/js/pwstrength.js"></script>
	<script src="assets/js/page/form-plugins.demo.min.js"></script>
	<!-- ================== BEGIN PAGE LEVEL JS ================== -->
	<script src="assets/plugins/form/bootstrap-wizard/js/bootstrap-wizard.min.js"></script>
	<script src="assets/js/page/form-wizards.demo.min.js"></script>
	<script src="assets/js/apps.min.js"></script>
	
	<!-- ================== BEGIN PAGE LEVEL JS ================== -->
	<script src="assets/plugins/form/bootstrap-tagsinput/js/bootstrap-tagsinput.min.js"></script>
	<script src="assets/plugins/form/summernote/summernote.min.js"></script>
	<script src="assets/js/page/email-compose.demo.min.js"></script>
	<script src="assets/js/apps.min.js"></script>
	<!-- ================== END PAGE LEVEL JS ================== -->
	<!-- ================== END PAGE LEVEL JS ================== -->
	
	<script>
		$(document).ready(function() {
			App.init();EmailCompose.init();
			FormWizards.init();FormPlugins.init();
		});
	</script>

</body>
</html>
