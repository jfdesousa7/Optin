...
if(isset($_GET["eliminar"])){
	eliminar('eventos',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
$update=mysqli_query($link,"update eventos    set  estatus='".$estatus."',

titulo='".limpiar($link,$_POST["titulo"])."',
texto='".limpiar($link,$_POST["texto"])."',fecha='".$_POST["fecha"]."'
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."eve".".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update eventos set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if (isset($_POST["btn_guardar"])){
 ...
 mysqli_query($link,"insert into eventos  values(NULL,NULL,NULL,NULL,NULL,'si')");
 $id=mysqli_insert_id($link);
	 
 $update=mysqli_query($link,"update eventos    set  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update eventos set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
<body>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			<a href="eventos-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="width:1%"></th>
						 <th class="no-sort" style="white-space: nowrap">Evento</th>
						 <th class="no-sort" style="white-space: nowrap">Fecha Evento</th>
...
$sql1=mysqli_query($link,"select * from  eventos ");
...
<a href="eventos-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
