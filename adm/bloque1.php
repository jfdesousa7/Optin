...
if (isset($_POST["btn_editar"])){
	...
	if( $_POST["hide_bloque_1"]==""){$estatus="si";}else{$estatus="no";}

		$update=mysqli_query($link,"update home set 
		
		bloque_1_texto2='".mysqli_real_escape_string($link,$_POST["bloque_1_texto2"])."',
		bloque_1_texto3='".mysqli_real_escape_string($link,$_POST["bloque_1_texto3"])."',
		hide_bloque_1='".$estatus."'
...
if($ext!=""){		
			 	            $actual_image_name ="bloque_1_imagen".date("H_i_s").".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
mysqli_query($link,"update home set bloque_1_imagen='".$actual_image_name."'  where id=".$_POST["id"]."") or die ("error subir foto ");
								  }
}else{
			...
			<div id="rootwizard" class="wizard wizard-full-width">
				...
				<div class="wizard-header">
					<ul class="nav nav-pills">
						<li class="active"><a href="#tab1" data-toggle="tab">Bloque 1</a></li>
...
<div class="form-group">
	<?php  if ($row1["hide_bloque_1"]=='no'){?>   <div class="switcher switcher-success">
	<input type="checkbox" name="hide_bloque_1" id="hide_bloque_1" checked="" value="no" />
	<label for="hide_bloque_1"></label>
	</div>  <?php }else{ ?>
	 <div class="switcher switcher-success">
	<input type="checkbox" name="hide_bloque_1" id="hide_bloque_1"  value="si" />
	<label for="hide_bloque_1"></label>
	</div> <?php } ?>	
</div>										
									...
									<div class="form-group">
										<label class="control-label">bloque_1_texto1</label>
										<input type="text" class="form-control" required name="bloque_1_texto2" value="<?=$row1["bloque_1_texto2"]?>" />
									</div>
									
									<div class="form-group">
										<label class="control-label">bloque_1_texto2</label>
										<input type="text" class="form-control" required name="bloque_1_texto3" value="<?=$row1["bloque_1_texto3"]?>" />
									</div>
									
									
									<div class="form-group">
										<label class="control-label">bloque_1_imagen</label>
										<input type="file" name="foto" >
										
										<br><hr>
<center><img src="<?=$RUTA?>/<?=$row1["bloque_1_imagen"]?>" class="img-responsive" style="width:200px"></center>
									</div>
