			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						...
						<th class="no-sort" style="width:1%"></th>
						<th class="no-sort" style="white-space: nowrap">Cliente</th>
						<th class="no-sort" style="white-space: nowrap">Order ID</th>
						<th class="no-sort" style="white-space: nowrap">No. of items</th>
						<th class="no-sort" style="white-space: nowrap">Total price </th>
						<th class="no-sort" style="white-space: nowrap">Date</th>
						<th class="no-sort" style="white-space: nowrap">Envío</th>
						<th class="no-sort" style="white-space: nowrap">Payment method</th>
						<th class="no-sort" style="white-space: nowrap">Order status</th>
				...
				<tbody>
				 ...
				 <?php   $cons=mysqli_query($link,"select * from orden order by id DESC");
					...
					<tr>
						...
						$cliente=consultar("select name from users where user_id=".$row["id_cliente"]."",$link);
						$cliente_email=consultar("select email from users where user_id=".$row["id_cliente"]."",$link);
					...
					<td class="bold"><?=$row["id"]?></td>
					 ...
					 <td class="bold"><?=strtoupper($cliente)?><br><p style="font-size:12px"><?=$cliente_email?></p></td>
...
<div class="col-md-4" style="background-color:#FFF"><h4><b>Orden Detalle</b></h4>
		<div class="row">
		 ...
		 <div class="col-md-12">
		<?php  $user=mysqli_fetch_assoc(mysqli_query($link,"select * from users where user_id=".$row["id_cliente"]."")); ?>  
		  ...
		  <?=$user["telefono2"]?><br>
		   ...
		   <?=$user["direccion"]?><br>
			 ...
			 <?=strtoupper($envio)?><br>
			 D.R= <?=$row["direccion_retiro"]?><br>
			 P.R=  <?=$row["persona_retiro"]?><br>
			  C.R=  <?=$row["cedula_retiro"]?><br>
			 T.R=  <?=$row["telefono_retiro"]?><br>
			 </fieldset>
			 ...
			 <hr>
				...
				Fecha Pago: <?=voltear($row["fecha_pago"])?><br>
				ID Confirmación: <?=$row["id_confirmacion"]?><br>
				Banco Procedente: <?=strtoupper($row["banco_pago"])?><br>
		  
		  <?=strtoupper($row["tipo_pago_pago"])?><br>
...
<div class="row">
                  ...
                  <div class="col-md-4"><a href=""><?=ucfirst(strtolower($product["nombre_es"]))?></a><br>
				    ...
				    <span><em>categorias:</em> <?=$categorias?></span>
					  ...
					  <span><em>tallas:</em> <b><?=$talla?></b></span>
				  
				  </div>
                  <div class="col-md-2"><?=fprecios($rr["precio"])?> </div>
                  <div class="col-md-2" align="right"> <?=$rr["cantidad"]?>   </div>
                  <div class="col-md-2"><?=fprecios($ptotal)?> </div>
