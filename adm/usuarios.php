...
if (isset($_POST["btn_editar"])){
		...
		nombres='".$_POST["nombres"]."',tipo_usuario='".$_POST["tipo_usuario"]."'
		,
...
if (isset($_POST["btn_clave"])){
	...
	$query = mysqli_query($link,"SELECT * FROM `_usuarios` WHERE  clave='".$_POST["clave"]."' and id=".$_POST["id"]." ") or die(mysqli_error());
...
if (isset($_POST["btn_guardar"])){
	...
	$clave="C".rand (10,2000)."ve";
	
	
	
		$update=mysqli_query($link,"insert into  _usuarios values ( NULL,
		'".$_POST["nombres"]."',
		
		'".$_POST["login"]."',
		'".$clave."',
		'activo','".$_POST["tipo_usuario"]."','".$_POST["email"]."',NULL,NULL
		
		)") or die ("Error ingresar".mysqli_error());	
		...
		$headers  = "MIME-Version: 1.0\r\n";
									...
									$headers .= 'From: Contraseña   <'.$informativos["correo1"].'>' . "\r\n";
									$message=' <html>
												<head>
												<title></title>
												</head>
												<body >
												<br>
												Saludos, Tu Contraseña es: '.$clave.'
												<br><br>
												<hr>
												
												<i>'.$informativos["nombre_pagina"].'</i><br>
												<i>'.$informativos["direccion"].'</i><br>
												<i>'.$informativos["telefono1"].'</i> , <i>'.$informativos["telefono2"].'</i><br>
												</body>
												</html>
												';
							mail($_POST["email"], " Contraseña  ", $message, $headers);
