<?php session_start(); include "../conex.php";
	...
	$consulta = "SELECT * FROM  proyectos_galeria where id_proyecto=".$_SESSION["id_prod"]." ORDER BY orden";
...
<head>
	<meta charset="utf-8" />
	<title>Proyectos Imagenes ordenar</title>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class="active">Proyectos Imágenes [Ordenar]</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
						...
						<ul id="lista" class="ui-sortable row" style=" width:auto; margin:auto">
								<?php 
                                    foreach ($elementos as $id => $nombre)
                                        echo '<li class="col-md-2" id="elemento-'.$id.'" contenteditable="false"><b><img style="width:100px" src="../uploads/'.$nombre.'"></b></li>';
                                ?>
						</ul>
		...
		$(function(){
			...
			lista.sortable({
                update: function(event,ui){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'proyectos-imagenes-controlador.php',
                        dataType: 'json',
                        data: {
                            accion: 'ordenar',
                            puntos: ordenPuntos
