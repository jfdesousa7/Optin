...
if(isset($_GET["eliminar"])){
	eliminar('proyectos_tipo',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
$update=mysqli_query($link,"update proyectos_tipo    set  estatus='".$estatus."', nombre='".$_POST["nombre"]."' 	 where id=".$_POST["id"]." ") or die ("Error al editar  ");	
$hide2="";
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."por".".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos_tipo set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if (isset($_POST["btn_guardar"])){
 ...
 mysqli_query($link,"insert into proyectos_tipo (id,nombre,fecha) values(NULL,NULL,NOW(),NULL)");
 $id=mysqli_insert_id($link);
	 
 $update=mysqli_query($link,"update proyectos_tipo    set   nombre='".$_POST["nombre"]."' where id=".$id." ") or die ("Error al guardar ");	
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos_tipo set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
<body>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			<a href="proyectos_tipo-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
...
$sql1=mysqli_query($link,"select * from  proyectos_tipo ");
...
<a href="proyectos_tipo-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
