...
if(isset($_GET["eliminar"])){
	eliminar('blog',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
$update=mysqli_query($link,"update blog    set   estatus='".$estatus."', 


titulo='".limpiar($link,$_POST["titulo"])."',  contenido='".$_POST["contenido"]."'
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."blg".".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update blog set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if (isset($_POST["btn_guardar"])){
 ...
 mysqli_query($link,"insert into blog values(NULL,NULL,NULL,NULL,NOW(),'si')");
 $id=mysqli_insert_id($link);
	 
 $update=mysqli_query($link,"update blog    set  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update blog set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
<body>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			<a href="blog-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
...
$sql1=mysqli_query($link,"select * from  blog order by id DESC");
...
<a href="blog-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
