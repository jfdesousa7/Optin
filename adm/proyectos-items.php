...
if(isset($_GET["eliminar"])){
	eliminar('proyectos_items',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";} 
$update=mysqli_query($link,"update proyectos_items    set 
		 estatus='".$estatus."',
		 nombre='".$_POST["nombre"]."', texto='".$_POST["texto"]."'		 
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."item".".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos_items set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
mysqli_query($link,"insert into proyectos_items values(NULL,".$_SESSION["id_prod"].",NULL,NULL,'si',NULL)");
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos_items set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class=""><a href="proyectos.php">Proyectos Items </a></li>
				<li class="active">Proyectos Items </li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			...
			<!-- END page-header -->
			<a href="proyectos-items-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
...
$sql1=mysqli_query($link,"select * from  proyectos_items where id_proyecto=".$_SESSION["id_prod"]." ");
$y=1; while($row1=mysqli_fetch_array($sql1)){ 

$item=consultar("select nombre from proyectos where id=".$row1["id_proyecto"]."",$link);
...
<a href="proyectos-items-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
