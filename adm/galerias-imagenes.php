...
if(isset($_GET["eliminar"])){
	eliminar('galeria_imagenes',$_GET["eliminar"],$link); $hide2="";
}
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update galeria_imagenes set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if (isset($_POST["btn_guardar"])){
	...
	mysqli_query($link,"insert into galeria_imagenes values(NULL,".$_SESSION["id_prod"].",NULL)");
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update galeria_imagenes set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class=""><a href="galerias.php">Galerías imagenes</a></li>
				<li class="active">Galerías imagenes</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			...
			<!-- END page-header -->
			<a href="galerias-imagenes-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
				...
				<thead>
					<tr>
						 <th class="no-sort" style="width:1%">#</th>
						  <th class="no-sort" style="width:1%"></th>
						 <th class="no-sort" style="white-space: nowrap">Galería</th>
...
$sql1=mysqli_query($link,"select * from  galeria_imagenes where id_galeria=".$_SESSION["id_prod"]." ");
$y=1; while($row1=mysqli_fetch_array($sql1)){ 

$item=consultar("select nombre from galeria where id=".$row1["id_galeria"]."",$link);
...
<a href="galerias-imagenes-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
