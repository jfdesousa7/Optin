...
if(isset($_GET["eliminar"])){
	eliminar('noticias',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
$update=mysqli_query($link,"update noticias    set  estatus='".$estatus."',
tipo='".$_POST["tipo"]."',
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."not".".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update noticias set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
$ext=extension($_FILES['boletin']['name']);
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."bol".".".strtolower($ext);
							$tmp = $_FILES['boletin']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update noticias set boletin='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
if (isset($_POST["btn_guardar"])){
 ...
 mysqli_query($link,"insert into noticias  values(NULL,NULL,NULL,NULL,NULL,NOW(),NULL,NULL,'si')");
 $id=mysqli_insert_id($link);
	 
 $update=mysqli_query($link,"update noticias    set  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update noticias set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update noticias set boletin='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
<body>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			<a href="noticias-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="white-space: nowrap">Titulo</th>
						 <th class="no-sort" style="white-space: nowrap">Imágen</th>
						 <th class="no-sort" style="white-space: nowrap">Boletin</th>
						 <th class="no-sort" style="white-space: nowrap">Video</th>
...
$sql1=mysqli_query($link,"select * from  noticias ");
...
<a href="noticias-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
...
<img  width="100px" height="100px" style="padding-right:5px;" src="<?=$RUTA?><?=$row1["foto"]?>"> 
                   <?php }?></td>  
						 <td  > <?php  if($row1["boletin"]==NULL){echo "NO HAY";}else{  ?>
<img  width="100px" height="100px" style="padding-right:5px;" src="<?=$RUTA?><?=$row1["boletin"]?>"> 
                   <?php }?></td>  
						<td><?=$row1["video"]?></td> 
