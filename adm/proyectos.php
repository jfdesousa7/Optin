...
if(isset($_GET["eliminar"])){
	eliminar('proyectos',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){
	if( $_POST["estatus"]==""){$estatus="no";}else{$estatus="si";}
	
$update=mysqli_query($link,"update proyectos    set   estatus='".$estatus."', 


id_tipo=".$_POST["id_tipo"].",
  nombre='".limpiar($link,$_POST["nombre"])."',
   video='".$_POST["video"]."',
    descripcion='".limpiar($link,$_POST["descripcion"])."',
	 link_360='".$_POST["link_360"]."',
	  link_brochure='".$_POST["link_brochure"]."',
	   metraje='".limpiar($link,$_POST["metraje"])."',
	    detalles='".limpiar($link,$_POST["detalles"])."',ubicacion='".$_POST["ubicacion"]."',ubicacion_portada='".$_POST["ubicacion_portada"]."',
		 amenidades='".limpiar($link,$_POST["amenidades"])."'
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."dir".".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos set foto='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if($_FILES['file']['name']!=""){ 
      $ext=explode(".",$_FILES['file']['name'])	;
	                        $actual_image_name =date("H_i_s")."file".".".strtolower($ext[1]);
							$tmp = $_FILES['file']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos set file='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								}
} // FIN FILE 
...
if (isset($_POST["btn_guardar"])){
 ...
 mysqli_query($link,"insert into proyectos values(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,'si',NULL,NULL,'center center',NULL)");
 $id=mysqli_insert_id($link);
	 
 $update=mysqli_query($link,"update proyectos    set  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos set foto='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
if($_FILES['file']['name']!=""){ 
      $ext=explode(".",$_FILES['file']['name'])	;
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update proyectos set file='".$actual_image_name."'  where id=".$id."") ;
								}
} // FIN FILE 		
...
<body>
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<!-- END page-header -->
			<a href="proyectos-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="white-space: nowrap">Proyecto</th>
						 
						 <th class="no-sort" style="white-space: nowrap">Descripción</th>
						<th class="no-sort" style="white-space: nowrap">Ubicación</th>
						 <th class="no-sort" style="white-space: nowrap">Imagen</th>
						 <th class="no-sort"  style="white-space: nowrap">Estatus</th>
...
$sql1=mysqli_query($link,"select * from  proyectos ");
$y=1; while($row1=mysqli_fetch_array($sql1)){ 

$id_tipo=consultar("select nombre from proyectos_tipo where id=".$row1["id_tipo"]."",$link);
...
<a href="proyectos-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>


<a href="proyectos-imagenes.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs" alt="IMAGENES"><i class="ti-image"></i></a>
<a href="proyectos-items.php?id=<?=$row1["id"]?>" class="btn btn-info btn-xs" alt="IMAGENES">Items</a>
<a onClick=" if(confirm('Esta seguro?')){window.location='<?=$_SERVER["PHP_SELF"]?>?eliminar=<?=$row1["id"]?>'};"  class='btn btn-default btn-xs'><i class="ti-close"></i></a>
						...
						<td><b><?=$row1["nombre"]?></b></td>
						<td><?=acortar($row1["descripcion"])?></td>
						<td><?=acortar($row1["ubicacion"])?></td>
