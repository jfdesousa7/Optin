...
if(isset($_GET["eliminar"])){
	eliminar('bloque_11_items',$_GET["eliminar"],$link); $hide2="";
}

if (isset($_POST["btn_editar"])){

	
		$update=mysqli_query($link,"update bloque_11_items    set 
...
if($ext!=""){		
			 	            $actual_image_name ="bloque_11_items".date("H_i_s").".".strtolower($ext);
							$tmp = $_FILES['foto']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
mysqli_query($link,"update bloque_11_items set foto='".$actual_image_name."'  where id=".$_POST["id"]."") or die ("error subir foto ");
								  }
}else{
...
if (isset($_POST["btn_guardar"])){
	...
	mysqli_query($link,"insert into bloque_11_items values(NULL,NULL,NULL,NULL)");
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
mysqli_query($link,"update bloque_11_items set foto='".$actual_image_name."'  where id=".$id."") or die ("error subir foto ");
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
	...
	<div id="page-container" class="page-header-fixed page-sidebar-fixed">
		...
		<div id="content" class="content">
			...
			<ul class="breadcrumb">
				...
				<li class="active">bloque_11_items</li>
			</ul>
			<!-- END breadcrumb -->
			<!-- BEGIN page-header -->
			...
			<!-- END page-header -->
			<a href="bloque_11_items-nuevo.php" class="btn btn-default btn-lg">Nuevo <i class="ti-pencil"></i></a>
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						...
						<th class="no-sort" style="white-space: nowrap">Contenido</th>
						 <th style="white-space: nowrap">Img. Ref.</th>
					     <th class="no-sort" style="width:1%"></th>
					</tr>
				</thead>
				<tbody>
...
$sql1=mysqli_query($link,"select * from  bloque_11_items  ");
...
<a href="bloque_11_items-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
