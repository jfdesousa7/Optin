...
$ext=extension($_FILES['fondo']['name']);
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."fondoser".".".strtolower($ext);
							$tmp = $_FILES['fondo']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update servicios set fondo='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
...
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."serF".".".strtolower($ext);
...
if($_FILES['link_brochure']['name']!=""){ 
	         
$ext=extension($_FILES['link_brochure']['name']);
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."brochure".".".strtolower($ext);
							$tmp = $_FILES['link_brochure']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update servicios set link_brochure='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
$ext=extension($_FILES['icono']['name']);
if($ext!=""){		
			 	            $actual_image_name =date("H_i_s")."serI".".".strtolower($ext);
							$tmp = $_FILES['icono']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update servicios set icono='".$actual_image_name."'  where id=".$_POST["id"]."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
if (isset($_POST["btn_guardar"])){
	...
	mysqli_query($link,"insert into servicios values(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL)");
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update servicios set fondo='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update servicios set link_brochure='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
...
if($ext!=""){		
							...
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update servicios set icono='".$actual_image_name."'  where id=".$id."") ;
								  }
}else{
	$hide=""; $sms="Imagen invalida, solo formatos .jpg o .png";
	}								  
			...
			<table id="datatables-default" class="table table-striped table-condensed table-bordered bg-white">
				<thead>
					<tr>
						 ...
						 <th class="no-sort" style="width:1%"></th>
						 <th class="no-sort" style="white-space: nowrap">Servicio</th>
						 <th class="no-sort" style="white-space: nowrap">Icono.</th>
						 <th class="no-sort" style="white-space: nowrap">Imágen Ref.</th>
						 <th class="no-sort" style="white-space: nowrap">Brochure.</th>
						 <th class="no-sort" style="white-space: nowrap">Cabecera.</th>
...
<a href="servicios-editar.php?id=<?=$row1["id"]?>" class="btn btn-default btn-xs"><i class="ti-pencil"></i></a>
						...
						<td  > <?php  if($row1["icono"]==NULL){echo "NO HAY";}else{  ?>
<img  width="100px" height="100px" style="padding-right:5px;" src="<?=$RUTA?><?=$row1["icono"]?>"> 
                   <?php }?></td>
						
						
					<td  > <?php  if($row1["foto"]==NULL){echo "NO HAY ";}else{  ?>
<img  width="100px" height="100px" style="padding-right:5px;" src="<?=$RUTA?><?=$row1["foto"]?>"> 
                   <?php }?></td>
				   
				   <td  > <?php  if($row1["link_brochure"]==NULL){echo "NO HAY ";}else{  ?>
<img  width="100px" height="100px" style="padding-right:5px;" src="<?=$RUTA?><?=$row1["link_brochure"]?>"> 
                   <?php }?></td>
				   
				    <td  > <?php  if($row1["fondo"]==NULL){echo "NO HAY ";}else{  ?>
<img  width="100px" height="100px" style="padding-right:5px;" src="<?=$RUTA?><?=$row1["fondo"]?>"> 
                   <?php }?></td>
