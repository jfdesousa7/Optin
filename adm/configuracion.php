...
if (isset($_POST["btn_editar"])){
		...
		telefono2='".mysqli_real_escape_string($link,$_POST["telefono2"])."',
		correo1='".mysqli_real_escape_string($link,$_POST["correo1"])."',
		correo2='".mysqli_real_escape_string($link,$_POST["correo2"])."',
		...
		youtube='".limpiar($link,$_POST["youtube"])."',
		linkedin='".limpiar($link,$_POST["linkedin"])."',
		 
		horario='".limpiar($link,$_POST["horario"])."',
		 
		nombre_pagina='".limpiar($link,$_POST["nombre_pagina"])."',
		descripcion_meta_generico='".limpiar($link,$_POST["descripcion_meta_generico"])."',
		palabras_claves='".limpiar($link,$_POST["palabras_claves"])."'
...
if($ext!=""){		
			 	            $actual_image_name ="logo".date("H:i:s").".".strtolower($ext);
							$tmp = $_FILES['photoimg']['tmp_name'];
							if(move_uploaded_file($tmp, $RUTA.$actual_image_name))
								{
									mysqli_query($link,"update _informativos set logo='".$actual_image_name."'  where id=".$_POST["id"]."") or die ("error subir foto ".mysqli_query());
								  }
}else{
...
<input value="<?=$row1["direccion"]?>" type="text"  required name="direccion" rows="3" class="form-control "  >
									...
									<div class="form-group">
										<label class="control-label">Teléfono 2</label>
										  <input type="text" value="<?=$row1["telefono2"]?>"  required name="telefono2" rows="3" class="form-control "  >
									</div>
									
									<div class="form-group">
										<label class="control-label">Correo 1 (Contacto Formulario)</label>
										  <input type="text" value="<?=$row1["correo1"]?>"  required name="correo1" rows="3" class="form-control "  >
									</div>
									
									
									<div class="form-group">
										<label class="control-label">Correo 2 (Solicitudes Formulario)</label>
										  <input type="text" value="<?=$row1["correo2"]?>"  required name="correo2" rows="3" class="form-control "  >
									</div>
									
									
									<div class="form-group">
										<label class="control-label">Twitter</label>
										  <input type="text" value="<?=$row1["twitter"]?>"   name="twitter" rows="3" class="form-control "  >
									</div>
									
									
									<div class="form-group">
										<label class="control-label">Facebook</label>
										  <input type="text" value="<?=$row1["facebook"]?>"   name="facebook" rows="3" class="form-control "  >
									</div>
									
									
									<div class="form-group">
										<label class="control-label">Instagram</label>
										  <input type="text" value="<?=$row1["instagram"]?>"   name="instagram" rows="3" class="form-control "  >
									</div>
									
									<div class="form-group">
										<label class="control-label">Youtube</label>
										  <input type="text" value="<?=$row1["youtube"]?>"   name="youtube" rows="3" class="form-control "  >
									</div>
									
									<div class="form-group">
										<label class="control-label">Linkedin</label>
										  <input type="text" value="<?=$row1["linkedin"]?>"   name="linkedin" rows="3" class="form-control "  >
									</div>
...
<img  style="padding-right:5px; width:200px; height:80px; float:right;background-color:#000" src="<?=$RUTA?>/<?=$row1["logo"]?>" > 
									...
									<br>
									<div class="form-group">
										<label class="control-label">Descripción Meta</label>
<textarea type="text"   name="descripcion_meta_generico" rows="3" class="form-control "  ><?=$row1["descripcion_meta_generico"]?></textarea>
									</div>
									
									<div class="form-group">
										<label class="control-label">Palabras claves</label>
<textarea type="text"   name="palabras_claves" rows="3" class="form-control "  ><?=$row1["palabras_claves"]?></textarea>
									</div>
